...
- Follow-up chunk: added default `basic_chase` AI controller integration, `last_tank_standing` mode rule winner resolution, and a minimal winner overlay in KorGE client rendering.

- Follow-up chunk: added tile structure collision hooks in `MapDefinition` and integrated map blocking checks into deterministic tank/projectile simulation + client placeholder rendering.

- Follow-up chunk: integrated weapon behavior dispatch (`basic_shell`, `triple_spread`) into deterministic firing flow and added a client-side `GameState` → `ServerSnapshotMessage` mapping bridge for protocol alignment.
