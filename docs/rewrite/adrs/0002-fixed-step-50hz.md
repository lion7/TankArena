...
- **Replay and network tick rates are decimated, not equal to the sim rate.** Snapshot frequency is decoupled from simulation frequency:
  - **Replay storage:** record one full `WorldSnapshot` every 10 ticks (**10 Hz**) plus per-tick `InputFrame`s and `GameEvent`s. The server is deterministic given inputs, so dense snapshots are redundant; 10 Hz keyframes are enough to seek and validate divergence.
  - **Network replication (future):** target **10 Hz** snapshot send rate over the wire (every 10 sim ticks), with per-tick input frames going server-bound and per-tick events coming from the server. The exact rate is a tuning knob owned by `RemoteMatchClient` ([ADR 0007](0007-authoritative-headless-kubriko-server.md)); `ReplayProtocol.tickRate` already carries a per-stream rate field.
...
- Replay files stay compact: snapshots at 10 Hz + per-tick inputs is roughly the same volume as snapshots at 50 Hz with no inputs, with stronger correctness guarantees.
