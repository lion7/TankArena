...
object LegacyAssetRegistry {
    ...
    fun resolveTankFrame(
        ...
        val legacyFrameNumber = legacyTankFrameNumber(
            facingX = facingX,
            facingY = facingY,
            animationFrame = animationFrame,
        )
        return sheet.frameAtLegacyIndex(legacyFrameNumber)
    }

    fun resolveTurretFrame(turretType: Int, direction: Int): SpriteFrameRef {
        val sheet = SpriteSheets.Towers
        val legacyFrameNumber = legacyTurretFrameNumber(direction)
        return SpriteFrameRef(
            sheet = sheet,
            column = (legacyFrameNumber - 1).coerceIn(0, sheet.columns - 1),
    ...
    private fun SpriteSheetDefinition.frameAtLegacyIndex(legacyFrameNumber: Int): SpriteFrameRef {
        val zeroBasedIndex = (legacyFrameNumber - 1).coerceAtLeast(0)
        return frameAt(normalizeFrameIndex(zeroBasedIndex, this))
    }
    ...
    private fun legacyTankFrameNumber(
        facingX: Int,
        facingY: Int,
        animationFrame: Int,
    ): Int {
        val baseDirectionFrame = when {
            facingY < 0 -> 1
            facingX > 0 -> 5
            facingY > 0 -> 9
            facingX < 0 -> 13
        ...
        val cycleOffset = when (animationFrame.mod(4)) {
            0 -> 0
            1 -> 1
            2 -> 0
            else -> 1
        }
        return baseDirectionFrame + cycleOffset
    }

    private fun legacyTurretFrameNumber(direction: Int): Int {
        val normalized = direction.mod(16)
