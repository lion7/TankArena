...
class LegacyAssetRegistryTest {
    ...
    @Test
    fun `maps tank cardinal directions to legacy frame groups`() {
        val north = LegacyAssetRegistry.resolveTankFrame(
            variant = 0,
            facingX = 0,
            facingY = -1,
            animationFrame = 0,
        )
        val east = LegacyAssetRegistry.resolveTankFrame(
            variant = 0,
            facingX = 1,
            facingY = 0,
            animationFrame = 0,
        )
        val south = LegacyAssetRegistry.resolveTankFrame(
            variant = 0,
            facingX = 0,
            facingY = 1,
            animationFrame = 0,
        )
        val west = LegacyAssetRegistry.resolveTankFrame(
            variant = 0,
            facingX = -1,
            facingY = 0,
            animationFrame = 0,
        ...
        assertEquals(0, north.column)
        assertEquals(0, north.row)
        assertEquals(4, east.column)
        assertEquals(0, east.row)
        assertEquals(0, south.column)
        assertEquals(1, south.row)
        assertEquals(4, west.column)
        assertEquals(1, west.row)
    }
